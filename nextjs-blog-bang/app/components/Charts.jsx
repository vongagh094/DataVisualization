...
export default function Charts() {
  ...
  useEffect(() => {
    ...
    const xScale = d3.scaleBand().domain(ageLabels).range([0, 400]).padding(0.2);
    const yScale = d3.scaleLinear().domain([0, d3.max(ageGroups.map(g => g.Yes + g.No))]).range([200, 0]);
    ...
    svg.selectAll('.bar-yes')
      ...
      .enter()
      .append('rect')
      .attr('x', (_, i) => xScale(ageLabels[i]) + 50 + xScale.bandwidth()/2)
